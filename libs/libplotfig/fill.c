/* This file contains the fill routine, which is an extension to GNU
...
/* Fig supports only 20 different values for fill levels, so we have to
   quantize.  See colordb.c. */
     ...
     int level;
{
  ...
  _drawstate->fig_fillcolor_defined = FALSE; /* lazily evaluated */
