...
static miZeroArcPt oob = {INT_MAX, INT_MAX, 0};

/* forward references */
static bool miZeroArcSetup ____P((const miArc *arc, miZeroArc *info, bool ok360));
static miIntPoint * miZeroArcPts ____P((const miArc *arc, miIntPoint *pts));
static void miZeroArcDashPts ____P((miGC *pGC, const miArc *arc, DashInfo *dinfo, miIntPoint *points, int maxPts, miIntPoint **evenPts, miIntPoint **oddPts));
...
static miIntPoint *
#ifdef _HAVE_PROTOS
miZeroArcPts (const miArc *arc, miIntPoint *pts)
#else
...
static void
#ifdef _HAVE_PROTOS
miZeroArcDashPts (miGC *pGC, const miArc *arc, DashInfo *dinfo, miIntPoint *points, int maxPts, miIntPoint **evenPts, miIntPoint **oddPts)
#else
...
#endif
{
  ...
  int k1, k3, dx, dy;
  int dashRemaining;
  miIntPoint *arcPts[4];	/* hope is OK */
  miIntPoint *startPts[5], *endPts[5];
  int deltas[5];
  miIntPoint *startPt, *pt, *lastPt, *pts;
...
void
#ifdef _HAVE_PROTOS
miZeroPolyArc (miGC *pGC, int narcs, const miArc *parcs)
#else
miZeroPolyArc (pGC, narcs, parcs)
     miGC *pGC;
     int narcs;
     const miArc *parcs;
#endif
{
  ...
  for (arc = parcs, i = narcs; --i >= 0; arc++)
    {
      if (!miCanZeroArc(arc))
	/* call wide polyarc function, which can do zero-width polyarcs
	   too, on this arc */
	miPolyArc(pGC, 1, arc);
      else
	{
  ...
  numPts = maxPts << 2;
  dospans = ((pGC->lineStyle != miLineSolid) 
	     || (pGC->fillStyle != miFillSolid)) ? true : false;
  if (dospans)
    {
      widths = (unsigned int *)mi_xmalloc(sizeof(unsigned int) * numPts);
      maxw = 0;
    }
  if (pGC->lineStyle != miLineSolid)
    {
      numPts <<= 1;
      ...
      miStepDash (pGC->dashOffset, &dinfo.dashIndexInit,
		  pGC->dash, pGC->numInDashList, &dinfo.dashOffsetInit);
    }

  points = (miIntPoint *)mi_xmalloc(sizeof(miIntPoint) * numPts);
  for (arc = parcs, i = narcs; --i >= 0; arc++)
    {
      if (miCanZeroArc(arc))
	{
	  ...
	  n = pts - points;
	  if (!dospans)
	    miPolyPoint(pGC, miCoordModeOrigin, n, points);
	  else
	    {
	      if (n > maxw)
		{
	      ...
	      miFillSpans (pGC, n, points, widths, false);
	    }
	  if (pGC->lineStyle != miLineDoubleDash)
	    continue;

	  if ((pGC->fillStyle == miFillSolid) ||
	      (pGC->fillStyle == miFillStippled))
	    pGC->fgPixel = pGC->bgPixel; /* already stashed fgPixel */

	  pts = &points[numPts >> 1];
	  oddPts++;
	  n = pts - oddPts;
	  if (!dospans)
	    miPolyPoint (pGC, miCoordModeOrigin, n, oddPts);
	  else
	    {
	      if (n > maxw)
		{
	      ...
	      miFillSpans (pGC, n, oddPts, widths, false);
	    }

	  if ((pGC->fillStyle == miFillSolid) ||
	      (pGC->fillStyle == miFillStippled))
	    pGC->fgPixel = fgPixel; /* restore */
	}
    }
