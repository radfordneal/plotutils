/* Known page sizes and their interpretation (included by pagetype.c, and
   ...
   square) are specified here in terms of inches.  Estimated margin sizes,
   for each page size, are the same as used by AutoCAD.
   ...
   display.  The origin for the coordinate system is the upper left corner
   of the xfig display, and the flipped-y convention is used (y increases
   downward).

   For the HP-GL[/2] output device, we choose the width of the HP-GL[/2]
   graphics display to be the same as for the PS output device.  That is,
   we take the size of the graphics display to be slightly smaller than the
   width of the page.  Note that the origin for the HP-GL[/2] coordinate
   system is not the lower left corner of the printed page.  It is the
   lower left corner of the `hard-clip region', a proper subrectangle of
   the printed page.  The hard-clip region, and its size and orientation,
   differs from device to device.  In fact most HP-GL[/2] devices by
   default plot in landscape mode, with x increasing down the page (or roll
   of paper).

   (There is a subtle difference between HP-GL and HP-GL/2: for HP-GL/2
   devices the lower left corner of the hard-clip region is by default the
   same as the default location of the so-called `scaling point' P1, but
   for HP-GL devices it is not.)

   The fifth element below is significant only in the HP-GL[/2] structure.
   It is the HP-GL/2 plot length (emitted in an HP-GL/2 `PS' instruction,
   and of importance mostly for roll plotters). */
...
const Pagedata _pagedata[NUM_PAGESIZES] =
{
  ...
  /* legal, 8.5" x 14"; not an ANSI size ? */
  /* we use square size = 8.0" */
  { "legal", false,
      {0.25, 8.25, 8.25, 0.25, 0.0}, /* fig */
      {0.25, 8.25, 3.0, 11.0, 0.0}, /* ps */
  ...
  /* ledger, 17" x 11" (rotated ANSI B); not an ANSI size ? */
  /* we use square size = 10.0" */
  { "ledger", false,
      {3.5, 13.5, 10.5, 0.5, 0.0},	/* fig */
      {3.5, 13.5, 0.5, 10.5, 0.0},	/* ps */
  ...
  { "a4", true,
      ...
      {0.235, 8.035, 1.945, 9.745, 0.0}, /* ps */
  ...
  { "a3", true,
      ...
      {0.495, 11.195, 2.92, 13.62, 0.0}, /* ps */
