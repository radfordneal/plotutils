...
/* SpanData: a foreground group of Spans's, and a background group */
typedef struct
{
  SpanGroup	fgGroup, bgGroup;
} SpanData;

/* If pixel is current foreground pixel, add the Spans to the foreground
   group (also subtract from background group if line style is
   miLineDoubleDash).  If pixel is _not_ current foreground pixel, add to
   background group and remove from foreground group.  This is the only
   function or macro in the module that compares two pixel values. */

#define AppendSpanGroup(pGC, pixel, spanPtr, spanData) { \
	SpanGroup   *group, *othergroup = NULL; \
	if (pGC->samePixel (pixel, pGC->fgPixel)) \
	{ \
