...
const Plotter _X_default_plotter = 
{
  ...
  /* capabilities */
#ifdef USE_LJ_FONTS_IN_X
  1, 1, 1, 1, 1, 1, 0, 0,	/* capability flags (see extern.h) */
#else
  1, 1, 1, 1, 1, 0, 0, 0,	/* capability flags (see extern.h) */
#endif
  false,			/* display device can justify text? */
  false,			/* can mix arcs and lines in stored paths? */
  AS_AXES_PRESERVED,		/* allowed scaling for circular arcs */
  AS_AXES_PRESERVED,		/* allowed scaling for elliptic arcs */
...
bool
#ifdef _HAVE_PROTOS
_X_init_plotter (Plotter *plotter)
#else
_X_init_plotter (plotter)
     Plotter *plotter;
#endif
{
  ...
  vanish_s = (const char *)_get_plot_param (plotter, "VANISH_ON_DELETE");
  if (strcasecmp (vanish_s, "yes") == 0)
    plotter->vanish_on_delete = true;
  else
    plotter->vanish_on_delete = false;
...
bool
#ifdef _HAVE_PROTOS
_X_terminate_plotter (Plotter *plotter)
#else
_X_terminate_plotter (plotter)
     Plotter *plotter;
#endif
{
  return true;
